    ...
    function(declare, lang, domConstruct, _Widget, _Templated, d3) {
		var draw = function() {
			...
			this.y = d3.scaleLinear()
				.domain(determineDomain(this.data, this.yDomain, this.yDomainDefault)).nice()
				.range([this.height - this.margin.bottom, this.margin.top]);

			this.xAxis = g => g
				.attr('class', 'xAxis')	
				.attr('transform', `translate(0,${this.height - this.margin.bottom})`)
				.call(d3.axisBottom(this.x).tickSize(0).tickSizeOuter(0).tickPadding(10));
			this.yAxis = g => g
				.attr('class', 'yAxis')
			...
			this.bar = this.svg.append('g')
				...
				.attr('x', d => self.x(self.format.title(d.name)))
		...
		updateChart = function(param_obj) {
			...
			if(param_obj.sort) {
				this.x.domain(this.data.sort(param_obj.order).map(d => d.name));
			}

			const t = this.svg.transition().duration(750);

			this.y.domain(determineDomain(this.data, this.yDomain, this.yDomainDefault)).nice()
				.range([this.height - this.margin.bottom, this.margin.top]);
